...
export function ReferralLinkCard({ code }: { code: string }) {
  ...
  async function copyLink(target: 'sure' | 'linescout') {
    try { await navigator.clipboard.writeText(links[target]); setStatus(target); notify('success', 'Referral link copied. You can paste it into your message.'); }
    catch { setStatus('error'); notify('error', 'Unable to copy the link. Select it and copy it manually.'); }
