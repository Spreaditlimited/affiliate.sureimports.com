'use client';
import { useToastNotice, notify } from '@/lib/useToastNotice';
...
export function DeveloperWorkspace({ initialCredentials }: { initialCredentials: Credential[] }) {
  ...
  async function createKey() {
    ...
    } catch (error) { setNotice(error instanceof Error ? error.message : 'Unable to create API key.', 'error'); }
  ...
  async function revoke(pidCredential: string) {
    ...
    const result = await response.json();
    if (!response.ok) return setNotice(result.message || 'Unable to revoke API key.', 'error');
    setCredentials((current) => current.map((item) => item.pidCredential === pidCredential ? { ...item, active: false } : item));
    setNotice(result.message);
    } catch { setNotice('Unable to revoke the key. Check your connection and try again.', 'error'); }
  }
