...
export async function getDashboardOverview(affiliateId: number) {
  const [referrals, conversions, available, pending, paid, recent, services, payoutAccounts] = await Promise.all([
    ...
    prisma.affiliate_program_services.findMany({ where: { active: true }, include: { currencyRates: { where: { active: true }, orderBy: { currency: 'asc' } }, unitRates: { where: { active: true }, orderBy: [{ currency: 'asc' }, { billingUnit: 'asc' }] }, eventRules: { where: { active: true }, orderBy: { sortOrder: 'asc' } } }, orderBy: [{ sortOrder: 'asc' }, { displayName: 'asc' }] }),
...
export async function getEarningsData(affiliateId: number, requestedPage = 1) {
  ...
  return {
    ...
    conversions: conversions.map((item) => ({
      ...
      service: item.sourceEventKey ? `${item.service.displayName} · ${item.sourceEventKey.toLowerCase().replaceAll('_', ' ')}` : item.service.displayName,
